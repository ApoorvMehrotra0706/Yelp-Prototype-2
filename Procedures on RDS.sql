...
VALUES (ID_check,Dishname_check, Price_check, Cuisine_check, ingredients_check, description_check, _imageURL);
set newID =(SELECT LAST_INSERT_ID());
SELECT * FROM BEVERAGES WHERE BeveragesID=newID;
commit;
END$$
DELIMITER ;
...
VALUES (ID_check,Dishname_check, Price_check, Cuisine_check, ingredients_check, description_check,_imageURL);
set newID =(SELECT LAST_INSERT_ID());
SELECT * FROM DESSERTS WHERE DessertsID=newID;
commit;
END$$
DELIMITER ;
...
VALUES (ID_check,Dishname_check, Price_check, Cuisine_check, ingredients_check, description_check, _imageURL);
set newID =(SELECT LAST_INSERT_ID());
SELECT * FROM MAIN_COURSE WHERE MainCourseID=newID;
commit;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE `updateBeveragesItems` (IN ID_check bigint, IN Dishname_check varchar(60),
 IN Price_check decimal(10,2) , IN Cuisine_check varchar(30), IN ingredients_check varchar(150), 
 IN description_check varchar(100), BeveragesID_check bigint)
BEGIN
declare _cuisineId int;
declare exit handler for sqlexception rollback;
start transaction;
...
SET RestaurantID=ID_check, Dishname=Dishname_check, Price=Price_check, CuisineID=_cuisineId, 
Main_Ingredients=ingredients_check, Description=description_check
WHERE BeveragesID= BeveragesID_check and RestaurantID = ID_check;
commit;
END$$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE `updateDessertsItems` (IN ID_check bigint, IN Dishname_check varchar(60),
 IN Price_check decimal(10,2) , IN Cuisine_check varchar(30), IN ingredients_check varchar(150), 
 IN description_check varchar(100), DessertsID_check bigint)
BEGIN
declare _cuisineId int;
declare exit handler for sqlexception rollback;
start transaction;
...
SET RestaurantID=ID_check, Dishname=Dishname_check, Price=Price_check, CuisineID=_cuisineId, 
Main_Ingredients=ingredients_check, Description=description_check
WHERE DessertsID= DessertsID_check and RestaurantID = ID_check;
commit;
END$$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE `updateMainCourseItems` (IN ID_check bigint, IN Dishname_check varchar(60),
 IN Price_check decimal(10,2) , IN Cuisine_check varchar(30), IN ingredients_check varchar(150), 
 IN description_check varchar(100), MainCourseID_check bigint)
BEGIN
declare _cuisineId int;
declare exit handler for sqlexception rollback;
start transaction;
...
SET RestaurantID=ID_check, Dishname=Dishname_check, Price=Price_check, CuisineID=_cuisineId, 
Main_Ingredients=ingredients_check, Description=description_check
WHERE MainCourseID= MainCourseID_check and RestaurantID = ID_check;
commit;
END$$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE `updateSaladsItems` (IN ID_check bigint, IN Dishname_check varchar(60),
 IN Price_check decimal(10,2) , IN Cuisine_check varchar(30), IN ingredients_check varchar(150), 
 IN description_check varchar(100), SaladsID_check bigint)
BEGIN
declare _cuisineId int;
declare exit handler for sqlexception rollback;
start transaction;
...
SET RestaurantID=ID_check, Dishname=Dishname_check, Price=Price_check, CuisineID=_cuisineId, 
Main_Ingredients=ingredients_check, Description=description_check
WHERE SaladsID= SaladsID_check and RestaurantID = ID_check;
commit;
END$$
DELIMITER ;
...
declare exit handler for sqlexception rollback;
start transaction;
DELETE FROM DESSERTS WHERE RestaurantID = ID_check AND DessertsID = DID_check;
commit;
END$$
DELIMITER ;
...
SET Dishname = _Dishname, Price = _Price, CuisineID = _CuisineID, 
Description = _Description, Price = _Price, ImageURL = _imageUrl
WHERE RestaurantID = _id AND DessertsID = _did;
END$$
DELIMITER ;
...
INSERT INTO EVENTS(EventName, RestaurantID, Description, EventStartTime, EventDate,
Location, Hashtags, EventEndTime) 
VALUES (1, 'Event1','Participation open' , '2020-10-10', '12:00', '18:00:01', '1700 N, First Street #273 San Jose 95112 California United States of America', '#Event1');

CALL `YELP`.`createEvents`(1, 'Event1','Participation open' , '2020-10-10', '12:00', '18:00:01', '1700 N, First Street #273 San Jose 95112 California United States of America', '#Event1');
...
-- Procedure to fetch Customer Details
DELIMITER $$
CREATE PROCEDURE `fetchCustomerDetails` (IN _orderID BIGINT)
BEGIN
