...
async function handle_request(msg, callback) {
  switch (msg.api) {
    case "signupRestaurant": {
      let res = {};
      Login.findOne(
        { emailID: msg.body.emailID, Role: "Restaurant" },
        (error, result) => {
          if (error) {
            res.status = 500;
    ...
    case "fetchOrderandDetails": {
      const { RestaurantID, pageNo, sortValue } = url.parse(
        msg.url,
        true
      ).query;
    ...
    case "fetchRegisteredCustomers": {
      ...
      try {
        ...
        const count = (await Events.findOne({ _id: eventID }))
    ...
    case "fetchMessages": {
      const { RestaurantID, CustomerID, pageNo } = url.parse(
        msg.url,
        true
      ).query;
      const results = [];
      let res = {};
      try {
        const result = await Messages.find({ RestaurantID })
          .limit(4)
          .skip(pageNo * 4)
          .exec();
        const count = await Messages.find({ RestaurantID }).countDocuments();
        const noOfPages = Math.ceil(count / 4);
        const results = [];
        results.push(result);
        results.push(noOfPages);
