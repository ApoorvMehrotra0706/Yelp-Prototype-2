...
async function handle_request(msg, callback) {
  switch(msg.api) {
    case 'signupCustomer' : {
      let res = {};
      Login.findOne({ emailID: msg.body.emailID, Role: 'Customer' }, async (error, result) => {
        if (error) {
          res.status = 500
    ...
    case 'loginCustomer': {
      let res = {};
      Login.findOne({ emailID: msg.body.emailID, Role: 'Customer' }, async (error, user) => {
        if (error) {
          res.status = 500;
    ...
    case 'updateContactInfo': {
      let res = {};
      Login.findOne({ _id: msg.body.user_id, Role: 'Customer' }, async (error, user) => {
        if (error) {
              ...
              Login.updateOne(
                ...
                (er, data) => {
                  if (er) {
                    ...
                    const payload = { _id: user._id, username: msg.body.newEmailID, role: user.Role };
                    const token = jwt.sign(payload, secret, {
                      expiresIn: 1008000,
                    });
                    Customer.updateOne(
    ...
    case 'submitReview': {
      ...
      review.save((e, data) => {
        if (e) {
          ...
          Restaurant.findOne({ RestaurantID: msg.body.RestaurantID }, (error, result) => {
            if (error) {
              res.status = 500;
              res.end = 'Network Error';
              callback(null, res);
    ...
    case 'fetchAllOrders': {
      const { CustomerID, filter1, filter2, sortOrder, pageNo } = url.parse(msg.url, true).query;
      let orderDetails = [];
      let res = {};
      if (filter1 !== 'All') {
                ...
                if (er) {
                  ...
                  custRegEvents.save((e, data1) => {
                    if (e) {
                      ...
                      Customer.findOne({ CustomerID: msg.body.CustomerID }, async (error1, result1) => {
                        if (error1) {
                          res.status = 500;
                          res.end = 'Network Error';
    ...
    case 'followUser': {
      let res = {};
      try {
        Customer.findOne({ CustomerID: msg.body.CustomerID }, async (error, result) => {
          if (error) {
            res.status = 500;
            res.end = 'Network Error';
            callback(null, res);
    ...
    case 'fetchSearchedYelpUser': {
      ...
      try {
        let customerData = [];
        let count = 0;
        const { CustomerID, searchString, searchCriteria, pageNo } = url.parse(msg.url, true).query;
